...
export const useDiagnosticCore = () => {
  ...
  stateManager.setVariable(
    'validateOrigins',
    serverValues?.origins.length > 0 &&
      (serverValues?.origins.includes(serverValues?.uiURL) || serverValues?.origins.includes('*')),
  );
  stateManager.setVariable('getClusterHealth', serverValues?.k8sHealth !== undefined);
  stateManager.hasWarnings =
    serverValues?.origins.length > 0 && serverValues?.origins.includes('*');
