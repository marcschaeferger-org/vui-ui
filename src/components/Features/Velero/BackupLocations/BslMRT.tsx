...
export function BslMRT({ fetching, setReload, items, customActions }: any) {
  ...
  const columns = useMemo<MRT_ColumnDef<any>[]>(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <>{row?.original?.spec?.credential && row?.original?.spec?.credential?.name}</>
        ),
      },
      {
        ...
        header: 'Key Name',
        Cell: ({ row }) => (
          <>{row?.original?.spec?.credential && row?.original?.spec?.credential?.key}</>
        ),
      },
      {
